...
print("=========================================")
print("👑 SyllabuswithRohit - V35 APEX EDITION")
print("=========================================")
...
<body hx-boost="true">
    ...
    <div id="selection-toolbar">
        ...
        <button onclick="saveQuote('{filename}', '{book_title}')" class="toolbar-btn" style="color:#FFDD00;">📝 Save Quote</button>
...
print("🔄 Installing Apex Engines across all books...")
for b in library:
    old_filepath = b['link']
...
# --- GENERATE HOMEPAGE (WITH NOTEBOOK SECTION) ---
cards = ""
...
<body hx-boost="true">
    ...
    <main class="max-w-6xl mx-auto px-6 py-16">
        ...
        <div id="bookmarks-section" class="mb-12 hidden">
    ...
    <script>
        (function() {{
            ...
            // Render Notebook (Quotes)
            let quotes = JSON.parse(localStorage.getItem('myQuotes') || '[]');
            if(quotes.length > 0) {{
                document.getElementById('notebook-section').classList.remove('hidden');
                let qContainer = document.getElementById('quotes-container');
...
subprocess.run(["git", "commit", "-m", "V35 Apex: Medium-style Selection, Dictionary & Font Vault"], check=True)
subprocess.run(["git", "push"], check=True)
print("🌟 APEX EDITION LIVE! This is now a top-tier reading application.")

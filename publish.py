...
print("=========================================")
print("🍃 SyllabuswithRohit - V41.2 MINIMALIST")
print("=========================================")
...
<body hx-boost="true">
    ...
    <div id="quoteGenModal">
        ...
        <div class="canvas-controls">
            ...
            <button onclick="nativeShareImage()" id="nativeShareBtn" class="bg-[#00e676] text-black px-6 py-3 rounded-full font-bold text-sm tracking-wider uppercase hover:bg-[#00c853] transition hidden">📤 Share to App</button>
        </div>
    </div>
    ...
    <script>
        (function() {{
            ...
            window.onscroll = () => {{
                ...
                navProgress.setAttribute('stroke-dasharray', `${{scrolled}}, 100`);
            ...
            window.toggleBionic = function() {{
                ...
                if (window.isBionic) {{ location.reload(); }} 
                else {{ let pTags = contentDiv.getElementsByTagName('p'); for (let p of pTags) {{ let html = p.innerHTML; p.innerHTML = html.replace(/(<[^>]+>)|([A-Za-z\u0900-\u097F]+)/g, function(match, tag, word) {{ if (tag) return tag; if (word.length <= 1) return word; let mid = Math.ceil(word.length / 2); return `<span class="bionic-word"><b>${{word.slice(0, mid)}}</b>${{word.slice(mid)}}</span>`; }}); }} window.isBionic = true; }}
            }};
    ...
    <script>
        (function() {{
            ...
            window.exportData = function() {{
                const data = {{ myBookmarks: localStorage.getItem('myBookmarks'), myQuotes: localStorage.getItem('myQuotes') }};
...
subprocess.run(["git", "commit", "-m", "V41.2 Fix: Corrected python f-string variable escape in canvas generation"], check=True)
subprocess.run(["git", "push"], check=True)
print("🌟 SYNTAX ERROR FIXED & LAUNCHED! Ab Canvas properly kaam karega.")
